...
object ChemTuring {
  ...
  /*
   * Compute the composition of all of the operations and cache it so we don't
   * do that every time sucessor is called.
   */
  private val _successor =
    (SKIP orElse MSKIP orElse READ orElse WRITE)

  /**
   * Get the successor for the given state.
   */
  def successor(state: State) = _successor(state)
